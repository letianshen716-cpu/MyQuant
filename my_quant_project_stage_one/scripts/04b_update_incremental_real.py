...
import QUANTAXIS.QAUtil.QADate_trade as QADate_trade 
...
def run_real_incremental_update():
    today_str = datetime.now().strftime('%Y-%m-%d')
    print(f"\n正在生成至 {today_str} 的最新日历")
    ...
    QADate_trade.trade_date_sse = date_strings
    ...
    print("-" * 50)
    print("\n请再次运行 check_data.py 进行质检")

if __name__ == '__main__':
    try:
        run_real_incremental_update()
